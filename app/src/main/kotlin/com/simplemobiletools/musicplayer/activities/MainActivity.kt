...
class MainActivity : SimpleMusicActivity() {
        ...
        binding.mainMenu.getToolbar().setOnMenuItemClickListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.btnInternal -> {
                    ...
                    // Set dialog title, message, and add EditText for user input
                    dialogBuilder.setTitle("Internal Storage")
                    dialogBuilder.setMessage("Enter text to save in internal storage:")
                    dialogBuilder.setView(input)

                    // Set positive button and its action
                    dialogBuilder.setPositiveButton("Save") { dialog, which ->
                        val text = input.text.toString()
                        // Save the text to internal storage
                        try {
                            val fileName = "my_text_file.txt"
                            val fos: FileOutputStream = openFileOutput(fileName, Context.MODE_PRIVATE)
                            fos.write(text.toByteArray())
                            fos.close()
                            // Optionally, you can show a Toast or perform other actions to indicate successful saving
                        } catch (e: Exception) {
                            e.printStackTrace()
    ...
    private fun alarmManager(){
        ...
        // Inflate your custom layout for time picker here or use default time picker

        builder.setPositiveButton("Set") { dialog, which ->
            // Get selected time from your time picker and set the alarm
            val alarmManager = getSystemService(Context.ALARM_SERVICE) as AlarmManager
            val intent = Intent(this, AlarmReciever::class.java)
            val pendingIntent = PendingIntent.getBroadcast(this, 0, intent, PendingIntent.FLAG_IMMUTABLE)

            // Set your alarm here using selected time
            // For example, to set alarm after 10 seconds from now
            val currentTime = System.currentTimeMillis()
            val alarmTime = currentTime + (10 * 1000) // 10 seconds
            alarmManager.setExact(AlarmManager.RTC_WAKEUP, alarmTime, pendingIntent)

            // You may also save the selected time to SharedPreferences or elsewhere
        }
